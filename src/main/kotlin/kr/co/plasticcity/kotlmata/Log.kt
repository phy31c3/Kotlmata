...
internal class Logs
{
	companion object
	{
		...
		const val STATE_RUN_ENTRY_PREDICATE = "%s [%s] Run entry via predicate action {signal: %s}"
		...
		const val STATE_RUN_INPUT_PREDICATE = "%s [%s] Run input signal predicate action {signal: %s}"
		...
		const val STATE_RUN_EXIT_PREDICATE = "%s [%s] Run exit via predicate action {signal: %s}"
